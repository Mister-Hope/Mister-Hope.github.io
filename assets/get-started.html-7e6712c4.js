...
</code></pre></div><p>在这里，<code>content</code> 暂时被设置成了空数组，它表示页面的内容，接下来我们就需要丰富和完善它。</p><h2 id="添加第一个组件——标题" tabindex="-1"><a class="header-anchor" href="#添加第一个组件——标题" aria-hidden="true">#</a> 添加第一个组件——标题</h2>`,5),N=t(`<blockquote><p><code>content</code> 的每个元素都为一个对象，该对象会最终渲染为一个组件。</p><p>每个配置对象有一个固定的键 <code>tag</code> 来决定渲染的组件。有效的 <code>tag</code> 值及对应的渲染结果如下:</p><ul><li><a href="#title">title</a>: 大标题</li><li>...</li></ul></blockquote><p>现在，我们需要为小程序页面添加第一个组件——标题。</p><p>为让小程序渲染它，我们需要为 content 数组添加第一个元素。为了让这个元素渲染为标题组件，我们需要设置元素为一个对象，并设置 <code>tag</code> 键的值为 <code>title</code>:</p><div class="language-yaml" data-ext="yml"><pre class="language-yaml"><code><span class="token key atrule">title</span><span class="token punctuation">:</span> 统一身份认证
<span class="token key atrule">author</span><span class="token punctuation">:</span> 张三
<span class="token key atrule">date</span><span class="token punctuation">:</span> 2021年11月1日
<span class="token key atrule">shareable</span><span class="token punctuation">:</span> <span class="token boolean important">true</span>
