...
</code></pre></div><p>切换回 master 分支后，再查看一个 <code>readme.txt</code> 文件，刚才添加的内容不见了! 因为那个提交是在 dev 分支上，而 master 分支此刻的提交点并没有变:</p><p><img src="`+g+`" alt="示例图" loading="lazy"></p><p>现在，我们把 dev 分支的工作成果合并到 master 分支上:</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code>$ <span class="token function">git</span> merge dev
...
</code></pre></div><p>现在，<code>master</code> 分支和 <code>feature1</code> 分支各自都分别有新的提交，变成了这样:</p><p><img src="`+m+`" alt="示例图" loading="lazy"></p><p>这种情况下，Git 无法执行“快速合并”，只能试图把各自的修改合并起来，但这种合并就可能会有冲突，我们试试看:</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code>$ <span class="token function">git</span> merge feature1
...
</code></pre></div><p>现在，<code>master</code> 分支和 <code>feature1</code> 分支变成了下图所示:</p><p><img src="`+k+`" alt="示例图" loading="lazy"></p><p>用带参数的 <code>git log</code> 也可以看到分支的合并情况:</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>$ <span class="token function">git</span> log <span class="token parameter variable">--graph</span> <span class="token parameter variable">--pretty</span><span class="token operator">=</span>oneline --abbrev-commit
...
* d1be385 init hello
<span class="token punctuation">..</span>.
</code></pre></div><p>远程分支的提交历史也是一条直线。</p><h2 id="rebase-小结" tabindex="-1"><a class="header-anchor" href="#rebase-小结" aria-hidden="true">#</a> Rebase 小结</h2><ul><li><p><code>rebase</code> 操作可以把本地未 <code>push</code> 的分叉提交历史整理成直线；</p></li><li><p><code>rebase</code> 的目的是使得我们在查看历史提交的变化时更容易，因为分叉的提交需要三方对比。</p></li></ul>`,139);function A(x,w){const s=e("Mermaid");return t(),p("div",null,[h,o(s,{id:"mermaid-300",code:""}),f])}const q=a(v,[["render",A],["__file","branch.html.vue"]]);export{q as default};
