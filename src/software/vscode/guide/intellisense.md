...
选择一个符号然后输入 `Alt + F12`，你就可以使用上下文菜单了。

![上下文菜单](./assets/context-menu.gif)

## 跳转到定义

选择一个符号然后输入 `F12`，你也可以使用上下文菜单或 `Ctrl + Click`

![跳转到定义](./assets/goto-definition.gif)

你可以使用 **转到 > 返回** 命令或快捷键 `Alt + ←` 来返回你的上个位置。
...
安装 ESLint 扩展，并根据你的需要配置。请查阅 ESLint 规范以获取有关其格式化规则和选项的详细信息。
