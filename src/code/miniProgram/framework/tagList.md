...
| desc      |  否   |      `String`       | 标题描述文字       | 该描述文字为页面的详情描述，当标题因字数设置缩写或只有内容关键词时填入，它具有极高的搜索权重 |
| action    |  否   | `String | Boolean`  | 左上角按钮函数名   | 填入按钮点击时所触发的函数名称，不填时默认执行返回，设置为 `false` 来隐藏默认的返回按钮      |
